/* This is Version 1.1 of the public distribution of the code for the auditory
   periphery model of:
   ...
   will need to uncomment line 269 of this code and comment out line 270, and then
   recompile the code.
...
// if (nrhs != 8)
// {
//  printf("deafcat2newme requires 8 input arguments.");
// };
//
// if (nlhs != 9)
// {
//  printf("deafcat2newme requires 9 output arguments.");
// };
//
...
// nrep = (int)nreptmp[0];
// if (nreptmp[0]!=nrep)
//  printf("nrep must an integer.\n");
// if (nrep<1)
//  printf("nrep must be greater that 0.\n");
//
//    binwidth = binwidthtmp[0];
// if (binwidth!=0.002e-3)
// {
//  printf("binwidth should be 0.002ms (Fs = 500kHz) for normal usage.\n");
// }
//
// reptime = reptimetmp[0];
// if (reptime<pxbins*binwidth)  /* duration of stimulus = pxbins*binwidth */
//  printf("reptime should be equal to or longer than the stimulus duration.\n");
//
...
//
// totalstim = (int)floor(reptime/binwidth);
//
//    outsize[0] = 1;
// outsize[1] = totalstim;
//
...
//
//static double anzilanybruce07(void *vv)
//{
//
...
// //Species
// species = (double)(*getarg(7));
//
// printf("zbcatmodel2007: Zilany and Bruce (JASA 2006, 2007) Cat Auditory Nerve Model\n");
//
//
// return an_zilanybruce2007(tdres,cf,spont,cohc,cihc,species,stim,sout,nstim);
//
//}
...
int an_zilanybruce2007(double binwidth, double cf, double spont, double cihc, double cohc, int species, int nrep, double *px, double *synout, int totalstim)
{
  ...
  double Get_tauwb(double, double, int, double *, double *, int);
  double Get_taubm(double, double, double, double *, double *, double *, int);
  double gain_groupdelay(double, double, double, double, int *, int);
  ...
  // timeout= makevector(totalstim);
  meout = makevector(totalstim);
  // c1filterout= makevector(totalstim);
  // c2filterout= makevector(totalstim);
  // c1vihc= makevector(totalstim);
  // c2vihc= makevector(totalstim);
  // ihcout= makevector(totalstim);
  // sptime  = makevector(nrep*totalstim);

#if DEBUG == 1
  printf("an_zilanybruce2007: Initialising...\n");
#endif
  ...
  bmorder = 3;
  Get_tauwb(cf, CAgain, bmorder, Taumax, Taumin, species);
  taubm   = cohc * (Taumax[0] - Taumin[0]) + Taumin[0];
  ratiowb = Taumin[0] / Taumax[0];
  /*====== Parameters for the signal-path C1 filter ======*/
  Get_taubm(cf, CAgain, Taumax[0], bmTaumax, bmTaumin, ratiobm, species);
  ...
  wbgain = gain_groupdelay(binwidth, centerfreq, cf, tauwb, grdelay, species);
  tmpgain[0]   = wbgain;
  lasttmpgain  = wbgain;
  /*===============================================================*/
  ...
  /*===============================================================*/
#if DEBUG == 1
  printf("an_zilanybruce2007: Starting loop \n");
  ...
  for (n = 0;n < totalstim;n++) { /* Start of the loop */
    if (n == 0) { /* Start of the middle-ear filtering section  */
      ...
      meout[n] = 0.5472 * mey3[n] / megainmax;
    }
    ...
    wb_gain = gain_groupdelay(binwidth, centerfreq, cf, tauwb, grdelay, species);
...
#if DEBUG == 1
    printf("ZilanyBruce07: %d, %.3g, %.3g, %.3g, %.3g, %.3g, %.2g, %.2f\n", n, \
	   meout[n], c1filterouttmp[n], c2filterouttmp[n], c1vihctmp[n], c2vihctmp[n], ihcouttmp[n], synouttmp[n]);
#endif
  ...
  */
  ...
  // freevector(timeout);
  freevector(meout);
  // freevector(c1filterout);
  // freevector(c2filterout);
  // freevector(c1vihc);
  // freevector(c2vihc);
  //freevector(ihcout);
...
double Get_tauwb(double cf, double CAgain, int order, double *taumax, double *taumin, int species)
{
...
/* -------------------------------------------------------------------------------------------- */
double Get_taubm(double cf, double CAgain, double taumax, double *bmTaumax, double *bmTaumin, double *ratio, int species)
{
...
double gain_groupdelay(double binwidth, double centerfreq, double cf, double tau, int *grdelay, int species)
{
