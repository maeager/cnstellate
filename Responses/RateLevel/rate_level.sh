...
find -type d  | tr -d '.|/' | sort -n | sed '/^$/d' > level.dat

rm -f tstellate.dat TS_ratelevel_centre.dat
{
for i in $(ls -- */rateplace.0.dat|sort -n)
do 
	grep '^50' -- $i | awk '{print $3}'
done
} > tstellate.dat
paste level.dat tstellate.dat > TS_ratelevel_centre.dat
rm -f tstellate.dat
...
paste level.dat dstellate.dat > DS_ratelevel_centre.dat
rm -f dstellate.dat
gnuplot ../DS_ratelevel.gnu

{
for i in $(ls -- */rateplace.1.dat|sort -n); do 
 grep '^50' -- $i | awk '{print $3}'; done
}> tuberculo.dat
paste level.dat tuberculo.dat > TV_ratelevel_centre.dat
rm -f tuberculo.dat
...
paste level.dat golgi.dat > G_ratelevel_centre.dat
rm -f golgi.dat
