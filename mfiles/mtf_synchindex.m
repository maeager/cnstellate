...
for ii=0:3
  SI=zeros(100,17); RT=zeros(100,17);
  for freqmod = 50:50:1200
%    phist = load([num2str(spl) "/" num2str(freqmod) "/periodhist." num2str(ii) ".dat"] );
    phist = load([num2str(spl) "/" num2str(freqmod) "/psth." num2str(ii) ".dat"] );
    period = 1000 / freqmod 
...
dlmwrite([num2str(spl) "/vsPSTH." num2str(ii) ".dat"],SI,"\t");
dlmwrite([num2str(spl) "/rayltestPSTH." num2str(ii) ".dat"],RT,"\t");
end
