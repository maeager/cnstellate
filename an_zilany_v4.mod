...
//Get Input arguments
   if(ifarg(8)!=1){  //Must be changed if more input arguments added
      hoc_execerror("ERROR: input syntax must be sout.an_zilany_v4( stim, tdres,  cf, spontrate, cihc,cohc, species,nrep)", 0);
      return 0;
   }
   ...
   return an_zilany_ver4(tdres,cf,spont,cihc,cohc,species,nrep,stim,sout,nstim);
...
//Get Instance Sout vector for synapse data
   nsout = vector_instance_px(vv, &synsouttmp);
   void** xx;
   xx = (void**)(&xspikes);
   *xx = (void*)0;
   if (ifarg(1)) {
...
//Get number of Reps and tdres
   ...
   sptime = ((double*) vector_vec(spks));      //Get array ptr to spks Vector
    ...
    for (register int irand=0; irand< NoutMax+1;++irand) randNums[irand]=scop_random();
    randBufIndex = 0;
